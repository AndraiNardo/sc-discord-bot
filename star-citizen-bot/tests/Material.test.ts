...
test('Material model properties', (t) => {
  const attributes = (Material as any).rawAttributes;
...
test('Material model configuration', (t) => {
  assert.strictEqual(Material.name, 'Material', 'Model name should be Material');
  assert.strictEqual((Material as any).tableName, 'materials', 'Table name should be materials');
});
